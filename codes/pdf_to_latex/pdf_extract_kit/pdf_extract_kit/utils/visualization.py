...
def crop_bboxes_by_class(
    image_path,
    bboxes,
    classes,
    target_class,
    return_as="numpy",  # "numpy" or "pil"
    save_dir=None  
):
    """
    Crop bounding boxes of a specific class and return them as separate images.

    Args:
        image_path (str or PIL.Image.Image): The input image or path to the image.
        bboxes (list): List of bounding boxes [x_min, y_min, x_max, y_max].
        classes (list): Class ID for each bounding box.
        target_class (int): Class to crop.
        return_as (str): "numpy" for NumPy arrays, "pil" for PIL.Image.Image objects.
        save_dir (str or None): If provided, save the crops to this folder.
    ...
    for i, (bbox, cls_id) in enumerate(zip(bboxes, classes)):
        ...
        # Save if requested
        if save_dir:
            filename = f"cls_{target_class}_crop_{i}.jpg"
