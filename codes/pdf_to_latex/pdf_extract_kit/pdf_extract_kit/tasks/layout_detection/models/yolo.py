...
import cv2
import torch
from pdf_extract_kit.pdf_extract_kit.registry import MODEL_REGISTRY
from pdf_extract_kit.pdf_extract_kit.utils.visualization import visualize_bbox, crop_bboxes_by_class
from pdf_extract_kit.pdf_extract_kit.dataset.dataset import ImageDataset
...
class LayoutDetectionYOLO:
    ...
    def predict(self, images, result_path, image_ids=None):
        ...
        """
        print("Starting layout detection on images... usnign updated function")
        results = []
        for idx, image in enumerate(images):
            ...
            if self.visualize:
                ...
                target = 3 # imaage class

                crops = crop_bboxes_by_class(image, boxes, classes, target_class=target)

                # create folder result_path/images/
                crop_save_dir = os.path.join(result_path, "images")
                ...
                # save each crop numerically
                for i, crop in enumerate(crops):
                    crop_path = os.path.join(crop_save_dir, f"{base_name}_{i}.png")
