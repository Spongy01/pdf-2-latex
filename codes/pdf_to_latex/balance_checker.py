...
def check_latex_balance(tex_file_path: str, output_dir: str = None, apply_fixes: bool = False) -> str:
    ...
    Returns:
        Path to the generated report file (or fixed content if apply_fixes=True)
    """
    ...
    if apply_fixes:
        ...
        report_text = checker.generate_detailed_report(fix_report.post_correction_report, fix_report.corrected_content)
        
        # Save report
        if output_dir is None:
            output_dir = os.path.dirname(tex_file_path)
        
        report_filename = f"{os.path.splitext(os.path.basename(tex_file_path))[0]}_balance_report.txt"
        ...
        report_text = checker.generate_detailed_report(report, content)
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("Usage: python balance_checker.py <latex_file> [output_dir]")
        sys.exit(1)
    
    tex_file = sys.argv[1]
    output_dir = sys.argv[2] if len(sys.argv) > 2 else None
    
    check_latex_balance(tex_file, output_dir)
