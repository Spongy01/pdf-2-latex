...
def format_with_gpt(
    ...
    page_breaks = re.findall(
        r"%---- Page End Break Here ---- Page : (\d+)", tex_file_contents
    )
    page_positions = {
        int(page): pos.start()
        for page, pos in zip(
...
"""
    ...
    if use_parallel:
        ...
        with ThreadPoolExecutor(max_workers=batch_size) as executor:
            ...
            print(
                f"Processing {parts} parts with parallel execution (batch size: {batch_size})..."
            )
            ...
            for idx, page in enumerate(tqdm(page_breaks[:parts])):
                try:
                    ...
                    if len(futures_list) >= batch_size:
                        for future in tqdm(
                            as_completed(futures_list), desc="Processing batch"
                        ):
                            try:
